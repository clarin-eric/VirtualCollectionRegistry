    ...
    protected void initFooter(final Modal modal) {
        AjaxFallbackLink<AjaxRequestTarget> buttonOk = new AjaxFallbackLink<>("button") {
            ...
            public void onClick(final Optional<AjaxRequestTarget> target) {
                if(action != null) {
                    fireEvent(action.getEvent(target.isPresent() ? target.get() : null, modal));
                } else {
                    fireEvent(new AbstractDialogEvent(EventType.OK, target.isPresent() ? target.get() : null, modal));
                }
            }        
        };
        ...
        AjaxFallbackLink<AjaxRequestTarget> buttonCancel = new AjaxFallbackLink<>("button") {
            @Override
            public void onClick(final Optional<AjaxRequestTarget> target) {
                fireEvent(new AbstractDialogEvent(EventType.CANCEL, target.isPresent() ? target.get() : null, modal));
            }        
        };
